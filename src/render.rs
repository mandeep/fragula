...
pub fn render_loop(mut surface: GlfwSurface,
    ...
    let vertex_shader = include_str!("vertex.glsl").to_string();
    let fragment_shader = create_fragment_shader(&fragment_path);
    let mut shader_program = create_shader_program(&vertex_shader, &fragment_shader);
            ...
            match event {
                ...
                WindowEvent::CursorPos(x, y) => {
                    if cursor_pressed {
                        ...
                        y_angle += (x - last_x) as f32 * 0.8;
                        x_angle += (y - last_y) as f32 * 0.8;
                        last_x = x;
                        last_y = y;
